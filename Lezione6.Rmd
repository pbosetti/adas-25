...
Si parte con una design matrix completa e la si fraziona mediante `fp_fraction()`, passando come argomento la parte destra della relazione definente, come formula:
...
Possiamo raffinare ulteriormente il modello rimuovendo i termini non significativi, realizzando poi anche il diagramma di Pareto per valutare la relativa importanza degli effetti:

```{r}
dm.lm <- lm(Y~A*B + C, data=dm)
pareto_chart(dm.lm)
```
Realizziamo ora il grafico di interazione, tenendo in ascissa il fattore con l'effetto maggiore:

```{r}
dm %>% 
  add_predictions(dm.lm) %>% 
  mutate(A=factor(A), C=factor(C)) %>% 
  ggplot(aes(x=B, y=pred)) + 
  geom_line(aes(linetype=A)) + 
  geom_point() +
  geom_point(aes(y=Y, color=A)) +
  facet_wrap(~C, labeller=label_both)
```

Quindi:

* Aumentare B aumenta sempre la resa, e lo stesso vale per A e per C (ma in misura minore)
* La resa massima si ha quando tutti e tre i fattori sono a livello alto
* L'interazione tra A e B si evince dal fatto che le rette non sono parallele; anche se hanno comunque una pendenza simile, il fatto che l'interazione `A:B` risulti significativa nell'ANOVA significa che la differente pendenza è **statisticamente significativa**.


# Impianto di filtrazione v.2

Richiamiamo il piano fattoriale per l'impianti di filtrazione e *facciamo finta* di aver eseguito solo metà delle prove, compatibilmente con un PF frazionato $2^{4-1}$ con relazione definente $I=ABCD$. 
...
Ci basta quindi tenere solo metà della tabella, cioè solo le righe in cui vale la relazione definente, ri-arrangiata come $D=ABC$:
...
```
Dato che i punti sono pochi, il grafico risulta poco chiaro e di interpretazione incerta. Generalmente un diagramma quantile-quantile (eventualmente in forma semi-normale come qui) richiede almeno una ventina di punti per essere affidabile. Quando i punti sono pochi, come in questo caso, è più utile il diagramma di Pareto:
...
Verifichiamo se sia necessaria una trasformazione di Box-Cox:

```{r}
car::boxCox(dmf.lm)
```

La trasformazione più adatta è quindi $y^*=y^{1/2}$, anche se l'intervallo di confidenza è molto ampio, cioè qualsiasi trasformazione ha un impatto inferiore al 5%:

```{r}
dmf.lm <- lm(sqrt(Y)~A*C + A*D, data=dmf)
```
...
dmf %>% 
  ...
  mutate(C=factor(C), D=factor(D)) %>% 
  ggplot(aes(x=A, y=pred^2)) + 
...
Confrontiamo questo modello con quello ricavato dal modello completo:

```{r}
dm %>% 
  select(A:Y) %>% 
  # Aggiungo le predizioni anti-trasformate per il modello completo:
  add_predictions(lm(1/sqrt(Y)~A*C + A*D, data=.), var="full") %>% 
  mutate(full=1/full^2) %>% 
  # Aggiungo le predizioni anti-trasformate per il modello frazionato:
  add_predictions(lm(sqrt(Y)~A*C + A*D, data=.), var="half") %>% 
  mutate(half = half^2) %>% 
  # pivot, in modo da avere anche il tipo di modello come estetica del plot:
  pivot_longer(-(A:Y), names_to = "model", values_to = "pred") %>% 
  ggplot(aes(x=A, y=pred)) + 
  geom_line(aes(linetype=factor(C), color=model)) +
  geom_point(aes(color=model)) + 
  facet_wrap(~D, labeller=label_both)
```

Cioè: pur essendo ricavato sulla metà delle osservazioni, il modello ottenuto dal piano fattoriale frazionato è comunque in buon accordo con quello ottenuto dal PF completo. 

> Di fatto, per PF da $2^5$ in su è molto comune ricorrere a frazionamenti, anche perché più lunghe sono le relazioni definenti meno gli alias risultano difficili da sciogliere (perché gli alias riguardano sempre più interazioni di alto grado, poco probabili in virtù del **principio di sparsità degli effetti**).

L'ultimo grafico può anche essere ottenuto usando `facet_grid()`, che consente di sfaccettare il grafico per due estetiche anziché una:
...
dm %>% 
  ...
  mutate(half = half^2) %>% 
  pivot_longer(-(A:Y), names_to = "model", values_to = "pred") %>% 
  ggplot(aes(x=A, y=pred, color=factor(C))) + 
  geom_line() +
  geom_point() + 
  facet_grid(model~D, labeller=label_both) + 
  labs(color="C")
```
