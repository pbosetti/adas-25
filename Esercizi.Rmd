...
Con la libreria `rsample`:

```{r}
library(rsample)
set.seed(0)
data.rs <- bootstraps(data, times = 10000) %>% 
  mutate(results = map(splits, ~ tryCatch(reg(analysis(.)), error = \(e) reg(data)) |> broom::tidy()))
data.rs %>% 
  int_pctl(results)
```
...
Definiamo una funzione per calcolare la resa:
...
set.seed(1)
fp <- fp %>% 
  mutate(C=ifelse(.treat=="center", ifelse(.rep %% 2 == 1, -1, 1), C)) %>% 
  rowwise() %>% 
  mutate(Y = f(A, B, C) %>% round(2)) %>% 
  ungroup()
...
fp %>% 
  ...
  ggplot(aes(x=A)) + 
  geom_line(aes(y=pred, group=B, color=B)) +
